...
namespace CustomGrid
{
    ...
    public class ColumnGrid : CustomGrid
    {
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            var Height = GetFillHeight(this) ? availableSize.Height : Math.Min(Children.Count == 0 ? 0 : Children.Max(x => x.DesiredSize.Height), availableSize.Height);
            ...
            IEnumerable<(UIElement Element, double Width)> ColumnWidth = definition.Select(x =>
                (
                    ...
                    ((Func<double>)delegate
                    {
                        switch (x.GridType)
                        {
                            case GridType.Auto: return x.Element.DesiredSize.Width;
                            case GridType.Pixel: return x.GridValue;
                            case GridType.Star: return Star * x.GridValue;
                            default: return 0;
                        }
                    }).Invoke()
                )
    ...
    public class RowGrid : CustomGrid
    {
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            var Width = GetFillWidth(this) ? availableSize.Width : Math.Min(Children.Count == 0 ? 0 : Children.Max(x => x.DesiredSize.Width), availableSize.Width);

            return new Size(Width, Star > 0 ? availableSize.Height : TotalPixel + TotalAuto);
        }

        protected override Size ArrangeOverride(Size finalSize)
            => Arranging(Children, finalSize);
            ...
            IEnumerable<(UIElement Element, double Width)> RowHeight = definition.Select(x =>
                (
                    ...
                    ((Func<double>)delegate
                    {
                        switch (x.GridType)
                        {
                            case GridType.Auto: return x.Element.DesiredSize.Height;
                            case GridType.Pixel: return x.GridValue;
                            case GridType.Star:
                                return Star * x.GridValue;
                            default: return 0;
                        }
                    }).Invoke()
                )
